...
OTHER_COLORS = dict(slider_tube="#2e3436",
...
def plot_fermisurface(data,
    ...
    # grid, from 0 to 1.
    a0, b0, c0 = np.meshgrid(np.arange(dims[0], dtype=np.float64) / dims[0],
                             np.arange(dims[1], dtype=np.float64) / dims[1],
                             np.arange(dims[2], dtype=np.float64) / dims[2],
                             indexing="ij")
    xyz0 = (a0[..., np.newaxis] * rlattvec[:, 0] +
    ...
    for i_a, i_b, i_c in itertools.product(range(-1, 1), repeat=3):
        xyz.append(xyz0 + i_a * rlattvec[:, 0] + i_b * rlattvec[:, 1] +
                   i_c * rlattvec[:, 2])
