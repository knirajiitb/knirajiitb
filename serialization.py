...
def save_calculation(filename, data, equivalences, coeffs, metadata):
    ...
    with lzma.open(filename, "wt", encoding="utf-8", preset=0) as f:
        ...
        # optimized in C.
        f.write(
            json.dumps([data, equivalences, coeffs, metadata],
                       cls=JSONEncoder))
...
def load_calculation(filename):
    ...
    if nruter[3]["format_version"] != BT2_FORMAT_VERSION:
        warning("unexpected bt2 format version ({} instead of {})".format(
...
def save_results(filename, data, fermi, Tr, mu0, mur, N, sdos, cv, cond,
    ...
    with lzma.open(filename, "wt", encoding="utf-8", preset=0) as f:
        f.write(
            json.dumps([
                data, fermi, Tr, mu0, mur, N, sdos, cv, cond, seebeck, kappa,
                hall, metadata
            ],
                       cls=JSONEncoder))
...
def load_results(filename):
    ...
    if nruter[12]["format_version"] != BTJ_FORMAT_VERSION:
        warning("unexpected btj format version ({} instead of {})".format(
...
def gen_bt2_metadata(data, derivatives):
    ...
    """
    return dict(program_version=PROGRAM_VERSION,
